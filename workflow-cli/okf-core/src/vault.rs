...
use crate::crud::search::{TextSearchError, TextSearchResult, search_text, search_text_matching};
...
/// The fixed default location of the Personal Vault: `~/.varde-workflow/`.
///
...
/// fall back to a relative `.varde-workflow/` in the current working
...
pub fn personal_vault_path() -> PathBuf {
    dirs::home_dir()
        .map(|home| home.join(".varde-workflow"))
        .unwrap_or_else(|| PathBuf::from(".varde-workflow"))
}

/// Which vault a filtering call narrows to.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum VaultSelector {
    /// The Personal Vault at [`personal_vault_path`] (`~/.varde-workflow/`).
    Personal,
...
pub fn search_filtered(
    ...
    match (vault, bundle) {
        ...
        (Some(VaultSelector::Personal), Some(_)) => {
            search_or_empty(&personal_vault_path(), filters)
        }
...
/// Combined exact-field and ranked-text search under vault resolution.
/// Both predicates are evaluated during one Concept traversal.
pub fn search_text_matching_filtered(
    vault: Option<VaultSelector>,
    bundle: Option<&Path>,
    query: &str,
    filters: &[(String, String)],
    ...
    match (vault, bundle) {
        (None, None) => {
            search_text_matching_merged(&project_root_fallback(), query, filters, limit)
        }
        (None, Some(p)) => search_text_matching_merged(p, query, filters, limit),
        (Some(VaultSelector::Personal), None) => {
            search_text_matching_or_empty(&personal_vault_path(), query, filters, limit)
        }
        (Some(VaultSelector::Personal), Some(p)) if p.is_relative() => {
            search_text_matching_or_empty(&personal_vault_path().join(p), query, filters, limit)
        }
        (Some(VaultSelector::Personal), Some(_)) => {
            search_text_matching_or_empty(&personal_vault_path(), query, filters, limit)
        }
        (Some(VaultSelector::Project), None) => {
            search_text_matching(&[project_root_fallback()], query, filters, limit)
        }
        (Some(VaultSelector::Project), Some(p)) => {
            search_text_matching(&[p.to_path_buf()], query, filters, limit)
        }
    }
}
...
fn search_text_matching_merged(
    project_bundle: &Path,
    query: &str,
    filters: &[(String, String)],
    limit: usize,
    ...
    bundles.push(project_bundle.to_path_buf());
    search_text_matching(&bundles, query, filters, limit)
}
...
fn search_text_matching_or_empty(
    bundle: &Path,
    query: &str,
    filters: &[(String, String)],
    limit: usize,
) -> Result<Vec<TextSearchResult>, TextSearchError> {
    match search_text_matching(&[bundle.to_path_buf()], query, filters, limit) {
        Ok(entries) => Ok(entries),
        Err(TextSearchError::ReadDirFailed { source, .. })
            if source.kind() == io::ErrorKind::NotFound =>
        {
...
pub fn show_merged(project_bundle: &Path, slug: &str) -> Result<ShownConceptWithSource, ShowError> {
...
fn show_merged_at(
    ...
    match show(project_bundle, slug) {
        ...
        Err(ShowError::Io(ConceptIoError::NotFound { .. })) => match show(personal_vault, slug) {
            Ok(concept) => Ok(ShownConceptWithSource {
                concept,
...
mod tests {
    ...
    fn personal_vault_path_points_at_dot_varde_docs() {
        assert!(personal_vault_path().ends_with(".varde-workflow"));
    }
    ...
    fn list_filtered_missing_personal_vault_is_empty_not_error() {
        with_home(|_home| {
            // No `.varde-workflow` under this HOME.
    ...
    fn lint_filtered_personal_relative_bundle_filters_subdirectory() {
        with_home(|home| {
            ...
            .unwrap();
            let report =
                lint_filtered(Some(VaultSelector::Personal), Some(Path::new("sub")), false)
                    .unwrap();
            let kinds: Vec<_> = report.issues.iter().map(|i| i.check_kind).collect();
            assert!(
                !kinds.contains(&CheckKind::BrokenLink),
