...
use crate::concept::{Concept, ConceptError, ConceptMetadataError};
...
pub enum CreateError {
    ...
    #[error(transparent)]
    Metadata(#[from] ConceptMetadataError),
...
/// body) with a non-empty description and sequence-valued aliases when
/// present. The slug is validated, then the bytes are written atomically as
/// `<slug>.md`.
...
pub fn create(bundle: &Path, bytes: &[u8], slug: &str) -> Result<CreatedConcept, CreateError> {
    ...
    // Re-check after creating missing ancestors. This catches an existing
    // symlink anywhere in the completed parent chain before writing.
    let path = concept_path_or(bundle, slug)?;
    match atomic_create(&path, bytes) {
        ...
        Err(source) => {
            return Err(CreateError::Io(ConceptIoError::WriteFailed {
                path,
                source,
            }));
        }
...
mod tests {
    ...
    fn concurrent_create_of_the_same_slug_never_clobbers_the_winner() {
        ...
        let doc_a = "---\ntype: decision\ndescription: A useful decision.\n---\n# A\n";
        let doc_b = "---\ntype: decision\ndescription: Another useful decision.\n---\n# B\n";
    ...
    #[test]
    fn create_rejects_missing_description_before_writing() {
        let dir = tempdir("okf-core-create-missing-description");
        let bytes = b"---\ntype: decision\n---\nbody\n";
        assert!(matches!(
            create(&dir, bytes, "missing-description").unwrap_err(),
            CreateError::Metadata(ConceptMetadataError::MissingDescription)
        ));
        assert!(!dir.join("missing-description.md").exists());
    }

    #[test]
    fn create_accepts_alias_sequence() {
        let dir = tempdir("okf-core-create-aliases");
        let bytes = b"---\ntype: decision\ndescription: A useful decision.\naliases:\n  - shortcut\n---\nbody\n";
        create(&dir, bytes, "with-alias").unwrap();
        assert_eq!(std::fs::read(dir.join("with-alias.md")).unwrap(), bytes);
    }
}
