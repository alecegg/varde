...
pub fn write_tick(state_path: &Path, dispatching: bool, pending_count: usize) {
    let last_dispatch_error = read(state_path).and_then(|state| state.last_dispatch_error);
    write_tick_with_error(state_path, dispatching, pending_count, last_dispatch_error);
}

/// Write a heartbeat and explicitly replace the last dispatch error.
pub fn write_tick_with_error(
    state_path: &Path,
    dispatching: bool,
    pending_count: usize,
...
mod tests {
    ...
    #[test]
    fn read_legacy_state_defaults_last_dispatch_error() {
        let state: WatcherState = serde_json::from_str(
            r#"{"last_tick_unix":1704067200,"dispatching":false,"pending_count":0}"#,
        )
        .unwrap();
        assert_eq!(state.last_dispatch_error, None);
    }

    #[test]
    fn write_tick_preserves_last_dispatch_error() {
        let dir = tempdir().unwrap();
        let path = dir.path().join("state.json");
        write_tick_with_error(
            &path,
            false,
            0,
            Some("spawn failed: claude not found".to_string()),
        );
        write_tick(&path, true, 1);

        assert_eq!(
            read(&path).and_then(|state| state.last_dispatch_error),
            Some("spawn failed: claude not found".to_string())
        );
    }
