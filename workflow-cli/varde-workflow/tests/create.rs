//! Integration tests for `varde-workflow concept create`.
...
const DOC: &str = "---\ntype: decision\ntitle: Use Rust\ndescription: A decision about Rust.\n---\n# Body\n\nRust for the CLI.\n";
...
mod create {
    ...
    #[test]
    fn missing_description_is_rejected_without_writing() {
        let bundle = temp_bundle("create-missing-description");
        let inputs = temp_inputs("create-missing-description");
        let input = inputs.join("missing-description.md");
        std::fs::write(&input, "---\ntype: decision\n---\n# Body\n").unwrap();
        ...
        assert!(!bundle.join("missing-description.md").exists());
    }

    #[test]
    fn scalar_aliases_are_rejected() {
        let bundle = temp_bundle("create-scalar-aliases");
        let inputs = temp_inputs("create-scalar-aliases");
        let input = inputs.join("scalar-aliases.md");
        std::fs::write(
            &input,
