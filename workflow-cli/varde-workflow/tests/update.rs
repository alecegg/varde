//! Integration tests for `varde-workflow concept update`.
...
mod update {
    ...
    fn happy_path_overwrites_and_reports_new_version() {
        ...
        assert!(
            text.contains(&format!("version {old} -> {new}")),
            "stdout: {text}"
        );
    ...
    fn json_flag_outputs_old_and_new_versions() {
        ...
        assert_eq!(
            parsed["old_version"],
            okf_core::occ::version(DOC.as_bytes())
        );
        assert_eq!(
            parsed["new_version"],
            okf_core::occ::version(DOC2.as_bytes())
        );
