//! Integration tests for `varde-workflow concept show`.
...
mod show {
    ...
    /// that `concept show` falls back to `$HOME/.varde-workflow/`.
    fn empty_home(tag: &str) -> PathBuf {
        temp_inputs(tag)
    }

    /// A HOME directory whose `.varde-workflow/` Personal Vault holds the
    /// given `slug` (with `DOC` content).
    fn home_with_personal_vault(tag: &str, slug: &str) -> PathBuf {
        let home = temp_inputs(tag);
        std::fs::create_dir_all(home.join(".varde-workflow")).unwrap();
        std::fs::write(home.join(".varde-workflow").join(format!("{slug}.md")), DOC).unwrap();
        home
    }
    ...
    fn git_derived_timestamps_appear_in_json_and_text_without_frontmatter_keys() {
        ...
        let text = String::from_utf8_lossy(&out.stdout);
        assert!(
            text.contains("created: 2020-01-01T00:00:00Z"),
            "stdout: {text}"
        );
        assert!(
            text.contains("updated: 2021-06-15T12:30:45+02:00"),
            "stdout: {text}"
        );
    ...
    fn personal_vault_fallback_show_still_gets_git_timestamps() {
        ...
        let personal = home.join(".varde-workflow");
    ...
    fn show_project_wins_over_personal_vault_collision() {
        ...
        std::fs::create_dir_all(home.join(".varde-workflow")).unwrap();
        std::fs::write(
            home.join(".varde-workflow/both.md"),
            b"---\ntype: personal\n---\npersonal body\n",
        )
        .unwrap();
