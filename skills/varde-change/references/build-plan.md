...
A user may ask to run the plan through step 6, then stop before review and
...
This skill runs **one plan**, executing tasks in dependency order in one
working location. Sequential execution remains the default. A `parallel`
strategy runs one proven conflict-free ready wave in isolated worktrees, then
verifies atomic integration before releasing dependents. Large work is split
into small plans at plan time.

1. **Read the references.** Read `references/build-plan-run.md` for plan and task
   discovery and execution. Read `references/build-interview.md` for harness
   detection, question format, and recommendation rules. Read
   `references/varde-code.md` only for discovery or relationship questions.
   Read known, bounded file content directly.
...
3. **Choose the execution location.** Use the current checkout by default, so
   ...
   `git check-ignore -q <working>/plans/<plan-id>/plan.md` once (treat a
   `<working>` outside the repository as ignored): if plan files are ignored
   they stay in the current checkout and never enter a commit; if tracked, they
   may move with a worktree and enter scoped commits.
   ...
   Resolve `execution=<auto|inline|fresh|parallel>` before work begins.
   Announce the selected execution strategy and task count. `inline` keeps
   every task in the current executor. `fresh` uses a new executor for every
   task. `parallel` dispatches one dependency-ready wave only after manifest
   ownership and verification-resource checks pass. `auto` uses `parallel`
   only when that proof succeeds, otherwise preserving the sequential choice.

4. **Read the plan, decompose it, and state a posture** before starting
   ...
   per task.
   **Decompose the spec+AC into tasks:** read `references/build-decomposition.md`
   ...
   block. Present the breakdown once and proceed when interactive. Run without
   pausing under `varde-change orchestrate` or an unattended named-plan run. Skip
   decomposition only when the plan already has task files.

5. **Execute tasks in dependency order, until none remain.** Sequential
   strategies choose one ready task. `parallel` chooses one ready wave, runs
   isolated workers, and recalculates readiness only after atomic integration.
   Delegate one task only when unfamiliar code or bounded investigation needs
   fresh context. Repeat until every task is `done` or `blocked`. Full loop,
   resume check, and bounded-retry rules live in
   `references/build-plan-run.md` and `references/build-execution.md`.

6. **Execute the selected task.** The executor reads
   ...
   `status: done` and choose the next ready task. There is no integration stage.

7. **Write knowledge gotchas, if any.** Create or edit a markdown note in the
   repo's existing knowledge location (e.g. `<knowledge>/`) directly.
...
8. **Review, verify plan-level AC, then complete.** Run one
   ...
   holds against the complete change, and toggle `- [ ]` to `- [x]` — an
