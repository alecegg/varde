...
into `## Open Questions` or `## Assumptions` (formats in `references/plan-grow-doc.md`). This
...
See `references/plan-authoring.md`'s Finish and prompt section.
...
items resolve or new ones show.
...
(gist only, no reasoning paragraph), written the moment the answer lands.
