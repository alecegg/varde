...
- The working tree is clean after the spike (no uncommitted spike files).
...
- Time-box: if the spike takes longer than 30 minutes, it's not a spike — mark the task `blocked` instead (per `references/build-execution.md`'s Blocker Handling), not the plan.
