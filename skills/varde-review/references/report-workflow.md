...
Steps 2-7 for varde-review.

## Resolve breadth and spec source
...
A plain review runs the default three — `CORRECTNESS`, `CODE`, `ARCHITECTURE`.
A request for a thorough or full review runs all ten, filtered by relevance. A
request naming one concern runs that category alone; if the named concern maps
to none of the ten, say which ten exist and ask rather than guessing.

State the resolved breadth and active categories before proceeding.

### Spec source

When `CORRECTNESS` is active, find the spec source in this order:
...
After creating the review folder, read the changed code directly. This is a
manual review — every finding comes from code you read:

1. Collect the diff for the target resolved in `references/report.md`'s
   `## Choose the target`: `git diff <base>...HEAD`, or `git diff HEAD` for
   uncommitted changes. Read every changed file in full, not just the hunk, so
   surrounding context is available. For an approved whole-codebase pass there
   is no diff — take the detected sections as the read list instead.
...
3. If the `varde-code` CLI is available (`references/varde-code.md`),
   ...
   `references/report-categories.md`.
...
If the request named an area, keyword, or path, filter sections to it. Warn and
stop if filtering produces no sections.
...
Load the `## Finding format` section of `references/report-format.md` before writing
...
refactor pass and wait for the user to take it up.
