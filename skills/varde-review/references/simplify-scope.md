...
Use this to find the changed lines this pass may edit. Do it before step 4 of
...
- Staged changes only: compare the index against `HEAD` — add `--cached` to both
  commands below.
- A named ref: compare against that ref instead of `HEAD` in both commands.
- Named paths: skip the name-status discovery step and treat each path as
  `status: modified` directly, but still compute its line ranges via the hunk
  diff below.
...
keeps the scope compact and removes one-line gaps.
