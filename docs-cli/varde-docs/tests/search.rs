//! Integration tests for `varde-docs concept search`.
...
mod search {
    ...
    fn fixture(bundle: &std::path::Path) {
        ...
        .unwrap();
        std::fs::write(bundle.join("pattern-b.md"), "---\ntype: pattern\n---\n# b\n").unwrap();
        std::fs::write(bundle.join("decision-c.md"), concept("recall_priority: low\n")).unwrap();
    }
    ...
    fn field_filter_returns_only_matching_slugs() {
        ...
        let out = search(&["--field", "type=decision", "--bundle", bundle.to_str().unwrap()]);
        assert!(
            out.status.success(),
            "stderr: {}",
    ...
    fn json_output_is_parseable_array_of_registry_entries() {
        ...
        let out = search(&["--field", "type=decision", "--json", "--bundle", bundle.to_str().unwrap()]);
        assert!(out.status.success());
