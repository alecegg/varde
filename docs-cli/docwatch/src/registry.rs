...
pub fn find<'a>(entries: &'a [WatcherRegistration], folder_or_id: &Path) -> Option<&'a WatcherRegistration> {
...
mod tests {
    ...
    fn add_duplicate_folder_errors() {
        ...
        add_entry(&path, &folder, PathBuf::from("/plist"), PathBuf::from("/logs")).unwrap();
        let err = add_entry(&path, &folder, PathBuf::from("/plist"), PathBuf::from("/logs"))
            .unwrap_err();
    ...
    fn add_entry_rejects_id_collision_with_different_folder() {
        ...
        let err = add_entry(&path, &folder_b, PathBuf::from("/pb"), PathBuf::from("/lb"))
            .unwrap_err();
