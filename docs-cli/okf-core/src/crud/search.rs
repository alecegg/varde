...
//! additively with [`crate::registry::search`]'s exact-value `--field`
//! filtering: callers narrow with `--field` first, then rank/filter the
//! remainder with this module (see `varde-docs/src/commands/search.rs`
//! for the composition).
//!
...
pub fn search_text(
    ...
    for bundle in bundles {
        let found = crate::walk::walk_concepts(bundle, &is_reserved)?;
        for crate::walk::WalkedFile { path, frontmatter, body } in found {
            ...
            let (chunk_score, preview) = score_body(&body, &terms);
            score += chunk_score;
...
fn score_text(text: &str, terms: &[String], weight: i64) -> i64 {
    ...
    terms.iter().filter(|term| normalized.contains(term.as_str())).count() as i64 * weight
}
...
mod tests {
    ...
    fn normalize_text_strips_punctuation_and_separators() {
        assert_eq!(normalize_text("Use-Rust_v1.0/beta\\x"), "use rust v1 0 beta x");
    ...
    fn no_match_returns_empty() {
        ...
        assert_eq!(search_text(&[dir], "zzz-nonexistent", 20).unwrap(), Vec::new());
    }
    ...
    fn aliases_and_tags_are_searchable() {
        ...
        let got = search_text(&[dir.clone()], "turbocharge", 20).unwrap();
        assert_eq!(got.len(), 1);
