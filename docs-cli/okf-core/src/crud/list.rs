...
pub fn list(bundle: &Path, include_deprecated: bool) -> Result<Vec<ListedConcept>, ListError> {
    let files = crate::walk::walk_concepts(bundle, &is_reserved_strict)?;
    let mut concepts = Vec::new();
    for crate::walk::WalkedFile { path, frontmatter, .. } in files {
...
mod tests {
    ...
    fn registry_list_no_type_no_error() {
        ...
        assert_eq!(
            registry_got.iter().map(|e| e.slug.as_str()).collect::<Vec<_>>(),
            vec!["bad"]
        );
