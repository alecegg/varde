//! Shared bundle-tree walk primitive for the abort-on-first-error query
...
/// The three ways a [`walk_concepts`] traversal can fail. Callers convert
...
pub enum WalkError {
    ...
    InvalidConcept { path: PathBuf, source: FrontmatterError },
}

/// Recursively walk `bundle`, returning every non-reserved `.md` Concept
/// file at any depth with its parsed frontmatter and body.
///
/// Hidden entries (dotfiles, `.git`), artifact directories (`target`,
/// `node_modules`), and symlinks are skipped. `reserved(name)` decides which
/// filenames are bundle bookkeeping rather than Concepts — pass
/// [`crate::bundle::is_reserved`] or a caller-specific predicate (e.g.
/// `crud::list`'s stricter `index.md`/`log.md`-only set). Aborts on the
/// first unreadable directory/file or malformed frontmatter.
pub fn walk_concepts(
    bundle: &Path,
    reserved: &dyn Fn(&str) -> bool,
) -> Result<Vec<WalkedFile>, WalkError> {
    let mut out = Vec::new();
    walk(bundle, reserved, &mut out)?;
    Ok(out)
}

fn walk(dir: &Path, reserved: &dyn Fn(&str) -> bool, out: &mut Vec<WalkedFile>) -> Result<(), WalkError> {
