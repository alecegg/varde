...
2. **Build and test within a folder.** `cd code-cli && cargo test`, `cd docs-cli && cargo test`, `cd skills && ./install.sh`, `cd agents && ./install.sh`. There is no root build or test entry point. `varde init` is the only root wiring entry point. It only wires supported harnesses through module installers.
3. **Names keep the `varde-` prefix.** Folders are short (`code-cli/`, `docs-cli/`, `skills/`, `agents/`), but package names, CLI names, and skill names retain their full `varde-*` identity.
...
are recorded in `memory-bank/friction/`.
