...
pub enum Command {
    ...
    /// JSON. Each flow summary carries a deduplicated `files` path table; its
    /// tree nodes reference paths by `f` index into that table rather than
    /// restating the path per node. (see `nav_map` help for inputs)
